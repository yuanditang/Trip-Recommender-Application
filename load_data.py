...
def get_demonyms():
    """
    Demonyms are used instead of country names in Wikipedia's URLS for visa requirements
    """
    country_demonyms = {
    #"Aruba": "Aruban",
    "Afghanistan": "Afghan",
...
def create_visa_matrix():
    ...
    for idx, row in visa_df.iterrows():
        ...
        country_name = row["destination_country"]
        cursor.execute("SELECT country_id, name FROM Country WHERE name = ?;", (country_name,))
        try:
            country_id = cursor.fetchall()[0][0]
            visa_df.at[idx, 'destination_country'] = country_id
        except IndexError:
            not_found.add(country_name)
        # Now change visa code to 0, 0.5, or 1
    ...
    visa_matrix = visa_df.pivot(index='origin_country_id', columns='destination_country', values='Visa requirement')
    # Add greenland (country_id = 75) as destination (just copy denmark's (country_id = 52) visa reqs)
    visa_matrix[75] = visa_matrix[52]
    visa_matrix.to_csv("cleaned_visa_requirements.csv", index=True)
...
def main():
    ...
    conn = sqlite3.connect('trip_recommender.db')
    cursor = conn.cursor()
    cursor.execute("PRAGMA table_info(VisaRequirement);")
